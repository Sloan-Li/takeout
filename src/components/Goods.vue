<template>
  <div class="goods-main">
    ...
    <div class="goods-list" id="goods" @scroll="goodsScroll()">
      <div
        class="goods-item"
        v-for="(goodItem, idx) in goods"
        :id="'goodItem' + idx"
        :key="idx"
      >
        <h3 class="goods-title" v-text="goodItem.name"></h3>
        <ul class="foods-list">
          <li v-for="(foodItem, index) in goodItem.foods" :key="index">
            <div
              class="food-img"
              @click="selecedFood(idx + '-' + index, foodItem)"
              :key="index"
            >
            ...
            <div class="food-context">
              ...
              <div class="food-price">
                ...
                <span class="old-price" v-show="foodItem.oldPrice"
                  >¥{{ foodItem.oldPrice }}</span
                >
              </div>
              <div class="change-goods-number">
                <a
                  class="link-change cut"
                  v-show="getFoodNum(idx + '-' + index) > 0"
                  @click="cutFoods(idx + '-' + index)"
                ></a>

                <span
                  class="num-span"
                  v-show="getFoodNum(idx + '-' + index) > 0"
                  v-text="getFoodNum(idx + '-' + index)"
                ></span>

                <a
                  class="link-change add"
                  @click="addFoods(idx + '-' + index, foodItem)"
                ></a>
              </div>
            </div>
          </li>
